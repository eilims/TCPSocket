...
int main(){
    ...
    while(1){
        ...
        if((socketDescriptorReference = accept(socketDescriptor, &clientAddr, &clientAddressLength) < 0){
            perror("Error: Accepting failed\n");
            exit(1);
        }

        while((bytesReceived = recv(socketDescriptorReference, bufferPointer, maxBytesToRead, 0)) > 0){
